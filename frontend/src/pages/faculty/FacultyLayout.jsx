...
import {
  LayoutDashboard, Brain, BarChart3, UserCircle2,
  History, GraduationCap, LogOut, Menu, X, ChevronRight,
} from 'lucide-react'

const navItems = [
  { to: '/faculty/dashboard', icon: LayoutDashboard, label: 'Dashboard',   color: { active: 'bg-indigo-50 text-indigo-700 border-indigo-200 dark:bg-indigo-500/15 dark:text-indigo-300 dark:border-indigo-500/30', dot: 'bg-indigo-500 dark:bg-indigo-400', icon: 'text-indigo-600 dark:text-indigo-400', hover: 'hover:bg-indigo-50 dark:hover:bg-indigo-500/10 hover:text-indigo-700 dark:hover:text-indigo-200' } },
  { to: '/faculty/quiz',      icon: Brain,           label: 'Quiz Builder', color: { active: 'bg-purple-50 text-purple-700 border-purple-200 dark:bg-purple-500/15 dark:text-purple-300 dark:border-purple-500/30', dot: 'bg-purple-500 dark:bg-purple-400', icon: 'text-purple-600 dark:text-purple-400', hover: 'hover:bg-purple-50 dark:hover:bg-purple-500/10 hover:text-purple-700 dark:hover:text-purple-200' } },
  { to: '/faculty/analytics', icon: BarChart3,       label: 'Analytics',   color: { active: 'bg-pink-50 text-pink-700 border-pink-200 dark:bg-pink-500/15 dark:text-pink-300 dark:border-pink-500/30',     dot: 'bg-pink-500 dark:bg-pink-400',   icon: 'text-pink-600 dark:text-pink-400',   hover: 'hover:bg-pink-50 dark:hover:bg-pink-500/10 hover:text-pink-700 dark:hover:text-pink-200'   } },
  { to: '/faculty/history',   icon: History,         label: 'Quiz History', color: { active: 'bg-cyan-50 text-cyan-700 border-cyan-200 dark:bg-cyan-500/15 dark:text-cyan-300 dark:border-cyan-500/30',     dot: 'bg-cyan-500 dark:bg-cyan-400',   icon: 'text-cyan-600 dark:text-cyan-400',   hover: 'hover:bg-cyan-50 dark:hover:bg-cyan-500/10 hover:text-cyan-700 dark:hover:text-cyan-200'   } },
  { to: '/faculty/account',   icon: UserCircle2,     label: 'Account',     color: { active: 'bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-500/15 dark:text-amber-300 dark:border-amber-500/30',  dot: 'bg-amber-500 dark:bg-amber-400',  icon: 'text-amber-600 dark:text-amber-400',  hover: 'hover:bg-amber-50 dark:hover:bg-amber-500/10 hover:text-amber-700 dark:hover:text-amber-200' } },
]
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-3 px-5 h-16 border-b border-gray-200 dark:border-gray-800/60 shrink-0">
        <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center shadow-lg shadow-purple-500/20 shrink-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[10px] text-purple-600/60 dark:text-purple-300/60 font-medium tracking-widest uppercase">Faculty</p>
        </div>
        {onClose && (
            ...
            >
              {({ isActive }) => (
                <>
                  ...
                  {isActive
                    ? <motion.span layoutId="faculty-nav-dot" className={`w-1.5 h-1.5 rounded-full ${c.dot}`} />
      ...
      <div className="p-3 border-t border-gray-200 dark:border-gray-800/60 shrink-0">
        <div className="flex items-center gap-3 p-3 rounded-xl bg-white dark:bg-gray-800/40 hover:bg-white dark:bg-gray-800/70 transition-colors">
          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center text-[11px] font-bold text-gray-900 dark:text-white shrink-0">
            {initials}
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-gray-900 dark:text-white truncate">{user?.username}</p>
            <p className="text-[10px] text-gray-600 dark:text-gray-500">Faculty</p>
          </div>
          <motion.button
        ...
        <header className="flex items-center gap-3 px-5 h-14 border-b border-gray-200 dark:border-gray-800/60 bg-gray-50 dark:bg-gray-900/40 backdrop-blur-sm shrink-0">
          ...
          <div className="flex items-center gap-2">
            <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-white dark:bg-gray-800/60 rounded-xl border border-gray-300 dark:border-gray-700/40">
              <div className="w-5 h-5 rounded-full bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center text-[9px] font-bold text-gray-900 dark:text-white">
                {initials}
              </div>
              <span className="text-xs text-gray-900 dark:text-gray-300 font-medium">{user?.username}</span>
            </div>
            <motion.button
